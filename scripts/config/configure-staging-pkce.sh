...
# Configure PKCE S256 for Staging Environment
# Run this after staging is deployed

set -e

echo "Configuring PKCE S256 for Staging..."
echo ""

# Check if staging container is running
if ! docker ps | grep -q peppertree_keycloak_staging; then
    echo "❌ Staging Keycloak container is not running"
    echo "   Start it with: docker compose -f docker-compose.staging.yml up -d"
    exit 1
fi

echo "✓ Staging Keycloak container is running"
echo ""

# Get admin token from staging Keycloak
...
CURRENT=$(docker exec peppertree_keycloak_staging curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
...
docker exec -i peppertree_keycloak_staging curl -s -X PUT "http://localhost:8080/admin/realms/peppertree/clients/$CLIENT_UUID" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d @-

echo ""
echo "✅ PKCE S256 configured successfully for STAGING"
echo ""
echo "Verification:"
docker exec peppertree_keycloak_staging curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:8080/admin/realms/peppertree/clients/$CLIENT_UUID" | \
  jq -r '.attributes["pkce.code.challenge.method"]'
echo ""
echo "Staging environment: https://staging.17peppertree.co.za"
